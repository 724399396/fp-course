...
mapParser f (P x) = P $ (\i -> case x i of
                                 ...
                                 UnexpectedChar l -> UnexpectedChar l
                                 UnexpectedString l -> UnexpectedString l)
...
list1 ::
  ...
  -> Parser (List a)
list1 pa =
  bindParser (\h -> bindParser (\t -> valueParser $ h :. t) (list pa)) pa
...
satisfy ::
  ...
  -> Parser Char
satisfy p =
  bindParser (\x -> if (p x) then valueParser x else unexpectedCharParser x) character
...
sequenceParser ::
  ...
  -> Parser (List a)
sequenceParser =
  foldRight (\pa pacc-> bindParser (\a -> bindParser (\acc -> valueParser $ a :. acc) pacc) pa) (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser =
  bindParser (\h -> bindParser (\t -> valueParser $ h :. t) $ list lower) upper
...
surnameParser ::
  Parser Chars
surnameParser =
  bindParser (\h -> bindParser (\t -> bindParser (\e -> valueParser $ h :. t ++ e) $ list lower) $ thisMany 5 lower) upper
...
phoneParser ::
  Parser Chars
phoneParser =
  bindParser (\h -> bindParser (\b -> (\_ -> valueParser $ h :. b) $ is '#') phoneBodyParser) digit
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser b
  pf <*> pa =
    bindParser (\f -> bindParser (\a -> pure $ f a) pa) pf
